...
public class ColumnarStructureX extends ColumnarStructure {
     ...
     * Confidence level   Tail Area   z critical
     *              90%   0.05        +-1.645
     *              95%   0.025       +-1.96
     *              99%   0.005       +-2.576
     * 
	...
	public float[] getNormalizedbFactors() {
		if (normalizedbFactors == null) {
		    ...
		    // TODO exclude waters to calculate statistics
			DescriptiveStatistics stats = new DescriptiveStatistics();
			for (float b : getbFactors()) {
				stats.addValue(b);
			}
	...
	public float[] getClampedNormalizedbFactors() {
		if (clampedNormalizedbFactors == null) {
			...
			for (int i = 0; i < getNumAtoms(); i++) {
				// normalize and scale the 90% confidence interval of the B factor
				// to [-1, 1].
				double s = normalizedbFactors[i] / 1.645;
				// set any value outside the 90% confidence interval to either -1 or
